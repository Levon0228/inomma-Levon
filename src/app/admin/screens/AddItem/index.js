...
import SingleItemForm from "./SingleItem";
  ...
  const handlePublish = () => {
    if (validateItems()) {
      dispatch(addItems(newItems.map(({ fields }) => {
        ...
        fields.startDate = new Date(Number(fields.startDate)).toLocaleDateString()
        fields.endDate = new Date(Number(fields.endDate)).toLocaleDateString()
        return fields;
      })));
      navigate("/admin");
    }
    ...
    <div>
      ...
      <Row>
        {newItems.map((item, index) => (
          <Col key={index} span={4} className={styles.col} >
        ...
        <Col span={6} className={c(styles.col, styles.addMore)}>
          <Button onClick={handleAddMore} type="ghost">Add Item +</Button>
        </Col>
      </Row>
      <div>
